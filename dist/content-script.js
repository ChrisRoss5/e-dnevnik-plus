...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (href.includes("/grade"))
        return addAverageBelowGradesTable();
...
function addAverageOnClass() {
    ...
    newItemContainer.appendChild(newItem).onclick = () => {
        ...
        Promise.all([...document.querySelectorAll(".content > .list a[href^='/grade/']")].map((subjectEl) => fetch(subjectEl.href)
            ...
            subjectEl.style.paddingRight = "17px";
...
function getSubjectFinalGrade(doc) {
...
function getSubjectAverage(doc) {
    return getAverageFromArray([...doc.querySelectorAll(".grades-table .grade")].flatMap((cell) => (cell.textContent.match(/\d/g) || []).map(Number)));
}
